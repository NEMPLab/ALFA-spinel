...
# 定义embedding模型
class BandGapEmbeder(nn.Module):
...
# 把元素信息矩阵和化学计量数信息矩阵相乘，输出矩阵看成组分信息
...
class OutEncoder(nn.Module):
    ...
    def forward(self, src, output, eg):
        mask = (src == 0).unsqueeze(-1).repeat(1, 1, self.d_model)
        output = output.masked_fill(mask, 0)
        output = torch.sum(output, dim=1, keepdim=True)
        eg = torch.reshape(eg, (eg.shape[0], 1))
        eg = self.embed(eg)
        eg = eg * 2 ** self.emb_scaler
        x = torch.cat([output, eg], dim=1)
        x = x.transpose(0, 1)
...
class CrabNet(nn.Module):
    ...
    def forward(self, src, frac, eg):
        # 元素和计量数
        output = self.encoder(src, frac)
        # 组分和带隙
        output = self.output_encoder(src, output, eg)
        eg = torch.reshape(eg, (eg.shape[0], 1))
        # average the "element contribution" at the end
        # mask so you only average "elements"
        mask = (torch.cat([eg, eg], dim=1) == 0).unsqueeze(-1).repeat(1, 1, self.out_dims)
