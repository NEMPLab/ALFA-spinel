...
from CrabNet_eg.utils import Lamb, Lookahead, RobustL1, BCEWithLogitsLoss, EDM_CsvLoader, Scaler, DummyScaler, \
    count_parameters
from CrabNet_eg.optim import SWA
    ...
    def train(self):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            y = y.to(self.compute_device,
                     dtype=data_type_torch,
                     non_blocking=True)
            eg = eg.to(self.compute_device,
                                         dtype=torch.float,
                                         non_blocking=True)

            output = self.model.forward(src, frac, eg)
            ...
            if learning_time:
                ...
                pred_v = np.nan_to_num(pred_v, copy=True, nan=0.0)
                if self.classification:
    ...
    def fit(self, epochs=None, checkin=None, losscurve=False):
        ...
        for epoch in range(epochs):
            ...
            if self.classification:
                ...
                pred_t = np.nan_to_num(pred_t, copy=True, nan=0.0)
                pred_v = np.nan_to_num(pred_v, copy=True, nan=0.0)
        ...
        if losscurve:
            ...
            xval[0] = 0
            var = pd.DataFrame(self.loss_curve)
            var.to_csv(self.model_name+'_loss_curve.csv')
            plt.plot(xval, self.loss_curve['train'],
