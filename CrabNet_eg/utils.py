...
from CrabNet_eg.composition import generate_features, _element_composition
...
class EDMDataset(Dataset):
    ...
    def __init__(self, dataset, n_comp):
        ...
        self.eg = np.array(self.data[3])

        self.shape = [(self.X.shape), (self.y.shape), (self.formula.shape), (self.eg.shape)]
    ...
    def __getitem__(self, idx):
        ...
        eg = torch.as_tensor(eg, dtype=data_type_torch)

        return (X, y, formula, eg)
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    eg = df['band gap'].values.astype(data_type_np)
